...
def sqdiff(image_part: np.ndarray, template: np.ndarray):
    sqdiff_sum = np.sum(
        (image_part.astype(np.float64) - template.astype(np.float64)) ** 2
    )
    return sqdiff_sum


def compute_mask_pixel_ratio(
    image_part: np.ndarray, template_mask: np.ndarray, image_mask_value: float
):

    image_mask_indexes = np.where(
        (image_part[:, :, 0] == image_mask_value)
        & (image_part[:, :, 1] == image_mask_value)
        & (image_part[:, :, 2] == image_mask_value)
    )
    ...
    true_mask_pixel_coords = npi.intersection(
        image_part_mask_pixel_coords, template_non_mask_pixel_coords
    )
...
def sqdiff_normed_modification(
    ...
    mask_pixel_ratio = compute_mask_pixel_ratio_v2(
        image_mask_part_binary, template_mask_binary, template_non_mask_pixels_count
    )
...
def cv_sqdiff_normed(
    image_part: np.ndarray, template: np.ndarray, mask: np.ndarray
) -> float:
