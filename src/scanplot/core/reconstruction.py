...
def insert_template_into_image(
        ...
        template_mask: np.ndarray | None = None) -> np.ndarray:
    """
...
def add_template_layer(
        ...
        bbox: Tuple[int, int, int, int],
        binary_template_mask: np.ndarray) -> np.ndarray:
    """
    ...
    template_non_mask_indexes = np.where(binary_template_mask != 0)  # TODO: передвать как аргумент
    x_indexes_template, y_indexes_template = template_non_mask_indexes
    x_indexes_image = x_indexes_template + x_min
    y_indexes_image = y_indexes_template + y_min
    
    layers_image[y_indexes_image, x_indexes_image] += binary_template_mask[y_indexes_template, x_indexes_template]
    return layers_image
